...
import type { SupervisoryAgentTypeOption } from '@/types/supervisory-agent'
...
<template>
  <form @submit.prevent="emit('submit')" class="space-y-6">
    ...
    <!-- Supervision details -->
    <fieldset class="space-y-4">
      <legend class="text-xs font-bold uppercase tracking-wide text-gray-400">Supervision details</legend>

      <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Supervision level</label>
          <select
            v-model="form.type"
            class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          >
        ...
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Agents supervised</label>
          <input
            v-model.number="form.agents_supervised"
            type="number"
            min="0"
            class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
          <p v-if="form.errors.agents_supervised" class="mt-1 text-xs text-red-600">{{ form.errors.agents_supervised }}</p>
        </div>
