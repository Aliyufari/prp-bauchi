...
import ReturningOfficerStats from './components/ReturningOfficerStats.vue'
import ReturningOfficerFilters from './components/ReturningOfficerFilters.vue'
import ReturningOfficerForm from './components/ReturningOfficerForm.vue'
import { columns } from './columns'
import { useDataTable } from '@/composables/table/useDataTable'
import type { ReturningOfficer, ReturningOfficerStatistics, ReturningOfficerFilterOptions } from '@/types/returning-officer'
...
interface Props {
  officers: DataTableResponse<ReturningOfficer>
  statistics: ReturningOfficerStatistics
  filterOptions: ReturningOfficerFilterOptions
...
function query() {
  router.get(
    route('returning-officers.index'),
    {
...
const editingOfficer = ref<ReturningOfficer | null>(null)
...
function emptyFormValues() {
  return {
    ...
    posting_location: '' as string | null,
...
function openEditModal(row: ReturningOfficer) {
  editingOfficer.value = row
  form.clearErrors()
  const values = {
...
function submitForm() {
  ...
  if (modalMode.value === 'create') {
    form.post(route('returning-officers.store'), options)
  } else if (modalMode.value === 'edit' && editingOfficer.value) {
    form.patch(route('returning-officers.update', editingOfficer.value.id), options)
  }
}
...
/* Row / bulk actions                                                     */
/* ---------------------------------------------------------------------- */
function viewOfficer(row: ReturningOfficer) {
  router.visit(route('returning-officers.show', row.id))
}

function handleRowAction(key: string, row: ReturningOfficer) {
  if (key === 'view') return viewOfficer(row)
  if (key === 'edit') return openEditModal(row)
  if (key === 'delete' && confirm(`Delete ${row.first_name} ${row.last_name}?`)) {
    router.delete(route('returning-officers.destroy', row.id), { preserveScroll: true })
  }
}

function handleBulkAction(key: string) {
  if (key === 'delete' && confirm(`Delete ${table.selected.value.length} returning officers?`)) {
    router.post(
      route('returning-officers.bulk-destroy'),
...
function toggleAll() {
  const ids = props.officers.data.map((o) => o.id)
...
<template>
  <Head title="Returning Officers" />
  <AppLayout>
    <div class="space-y-4 sm:space-y-6">
      <!-- Header -->
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
        <div class="min-w-0">
          <h2 class="text-lg font-black text-gray-900 sm:text-xl">Returning Officers</h2>
          <p class="mt-1 text-xs font-medium text-gray-500 sm:text-sm">
            Manage INEC-appointed returning officers and their postings
          </p>
        </div>
        <button
          type="button"
      ...
      <!-- Stats -->
      <ReturningOfficerStats :statistics="statistics" />

      <!-- Toolbar + filters -->
      <DataTableToolbar
        v-model="table.search.value"
        placeholder="Search by name, email, phone, institution or posting..."
        show-export
        show-refresh
        @refresh="query"
        @export="router.get(route('returning-officers.export'), { ...table.filters.value })"
      >
        <template #filters>
          <ReturningOfficerFilters v-model="table.filters.value" :options="filterOptions" />
        </template>
      </DataTableToolbar>
          ...
          >
            <template #officer="{ row }">
              <UserAvatarCell
                ...
                :subtitle="row.officer_no"
              />
            </template>
            <template #institution="{ row }">
              <span class="text-sm text-gray-700">{{ row.institution ?? '—' }}</span>
            </template>
            <template #posting="{ row }">
              <span class="text-sm text-gray-700">{{ row.posting_location ?? '—' }}</span>
            </template>
      ...
      <!-- Pagination -->
      <DataTablePagination
        :current-page="officers.meta.current_page"
        :last-page="officers.meta.last_page"
        :from="officers.meta.from"
        :to="officers.meta.to"
        :per-page="officers.meta.per_page"
        :total="officers.meta.total"
    ...
    <SlideOver
      :open="modalMode !== null"
      :title="modalMode === 'create' ? 'Add Returning Officer' : 'Edit Returning Officer'"
      :description="
        modalMode === 'create'
          ? 'Register a new returning officer and their posting.'
          : editingOfficer
            ? `Update details for ${editingOfficer.first_name} ${editingOfficer.last_name}.`
            : ''
      "
      size="lg"
      side="right"
      @close="closeModal"
    >
      <ReturningOfficerForm
        v-if="modalMode"
        :form="form"
        :submit-label="modalMode === 'create' ? 'Create officer' : 'Save changes'"
