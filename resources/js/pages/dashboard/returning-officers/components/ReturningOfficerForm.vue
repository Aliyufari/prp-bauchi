  ...
  <form @submit.prevent="emit('submit')" class="space-y-6">
    ...
    <fieldset class="space-y-4">
      ...
      <div v-if="'password' in form" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Officer ID</label>
          <input
    ...
    <!-- Posting details -->
    <fieldset class="space-y-4">
      <legend class="text-xs font-bold uppercase tracking-wide text-gray-400">Posting details</legend>

      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Institution</label>
          <input
            v-model="form.institution"
            type="text"
            placeholder="e.g. INEC"
            class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
          <p v-if="form.errors.institution" class="mt-1 text-xs text-red-600">{{ form.errors.institution }}</p>
        </div>

        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Posting location</label>
          <input
            v-model="form.posting_location"
            type="text"
            placeholder="e.g. Bauchi"
            class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          />
          <p v-if="form.errors.posting_location" class="mt-1 text-xs text-red-600">{{ form.errors.posting_location }}</p>
        </div>
      </div>
