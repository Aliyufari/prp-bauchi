...
import type { CoordinatorTypeOption } from '@/types/coordinator'
  ...
  <form @submit.prevent="emit('submit')" class="space-y-6">
    ...
    <fieldset class="space-y-4">
      ...
      <div v-if="'password' in form" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Coordinator ID</label>
          <input
    ...
    <!-- Coordinator details -->
    <fieldset class="space-y-4">
      <legend class="text-xs font-bold uppercase tracking-wide text-gray-400">Coordinator details</legend>

      <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Coordinator level</label>
          <select
            v-model="form.type"
            class="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500"
          >
    ...
    <!-- Jurisdiction -->
    <fieldset class="space-y-4">
      <legend class="text-xs font-bold uppercase tracking-wide text-gray-400">Jurisdiction</legend>
      ...
      <p class="text-xs text-gray-400">
        Set the jurisdiction level matching this coordinator's role — e.g. a State Coordinator
        typically only needs State set, while an LGA Coordinator needs State + LGA.
      </p>
    </fieldset>
